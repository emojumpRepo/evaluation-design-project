...
export class SurveyController {
  ...
  async createSurvey(
    ...
    const { title, remark, surveyCode, createMethod, createFrom, groupId, questionList, isCollaborated, pageConf, descriptionConfig } =
      value;
  ...
  async getExcelQuestions(@UploadedFiles() files: Express.Multer.File[]) {
    try {
      ...
      for (const file of files) {
        ...
        console.log('getExcelQuestions - Excel工作表列表:', workbook.SheetNames);
        ...
        if (workbook.SheetNames.includes('分页配置')) {
          ...
          pageConfData.forEach((row: any) => {
            ...
            if (row.字段 === '分页配置' && row.值) {
              pageConf = row.值.split(',').map((num: string) => parseInt(num.trim())).filter((num: number) => !isNaN(num));
              console.log('getExcelQuestions - 解析的分页配置:', pageConf);
            }
          });
        } else {
          console.log('getExcelQuestions - 未找到分页配置表，可用工作表:', workbook.SheetNames);
        }
        ...
        if (workbook.SheetNames.includes('描述配置')) {
          ...
          descriptionData.forEach((row: any) => {
            ...
            if (row.页面 && row.描述内容) {
              ...
              console.log('getExcelQuestions - 解析的描述配置:', pageKey, row.描述内容);
            }
          });
        } else {
          console.log('getExcelQuestions - 未找到描述配置表，可用工作表:', workbook.SheetNames);
        }
  ...
  async createSurveyFromExcel(
    ...
    try {
      ...
      const dataList = questions.map((excelQuestion, index) => {
        ...
        const question: any = {
          isRequired: excelQuestion.isRequired === '是' || excelQuestion.isRequired === 'true' || excelQuestion.isRequired === '1',
          showIndex: excelQuestion.showIndex === '是' || excelQuestion.showIndex === 'true' || excelQuestion.showIndex === '1',
          showType: excelQuestion.showType === '是' || excelQuestion.showType === 'true' || excelQuestion.showType === '1',
          showSpliter: excelQuestion.showSpliter === '是' || excelQuestion.showSpliter === 'true' || excelQuestion.showSpliter === '1',
          ...
          quotaDisplay: excelQuestion.quotaDisplay === '是' || excelQuestion.quotaDisplay === 'true' || excelQuestion.quotaDisplay === '1',
          cascaderData: {
            placeholder: [],
            children: [],
          },
        ...
        if (excelQuestion.options) {
          const optionTexts = excelQuestion.options.split(';').map((text: string) => text.trim()).filter(Boolean);
          const optionScores = excelQuestion.scores ? excelQuestion.scores.split(';').map((score: string) => score.trim()) : [];
          const othersOptions = excelQuestion.others ? excelQuestion.others.split(';').map((text: string) => text.trim()).filter(Boolean) : [];
          const mustOthersOptions = excelQuestion.mustOthers ? excelQuestion.mustOthers.split(';').map((text: string) => text.trim()) : [];
          const othersKeyOptions = excelQuestion.othersKey ? excelQuestion.othersKey.split(';').map((text: string) => text.trim()) : [];
          const placeholderDescOptions = excelQuestion.placeholderDesc ? excelQuestion.placeholderDesc.split(';').map((text: string) => text.trim()) : [];
          
          question.options = optionTexts.map((text: string, optionIndex: number) => {
            ...
            return {
              ...
              mustOthers: isOthersOption && (mustOthersOptions[optionIndex] === '是' || mustOthersOptions[optionIndex] === 'true'),
              othersKey: isOthersOption ? (othersKeyOptions[optionIndex] || `data${Date.now()}${index}_${optionIndex}`) : '',
              placeholderDesc: isOthersOption ? (placeholderDescOptions[optionIndex] || '') : '',
              hash: `${Date.now()}${index}${optionIndex}`,
              score: optionScores[optionIndex] && optionScores[optionIndex] !== '' ? parseInt(optionScores[optionIndex]) : undefined,
            };
          });
        }
  ...
  private validateExcelFile(file: Express.Multer.File): {
    ...
    let sheetName = workbook.SheetNames.find(name => name === '题目列表') || workbook.SheetNames[0];
    ...
    const isValidFormat = headerA1 === '题目标题' && headerB1 === '题型' && headerC1 === '选项内容';

    if (!isValidFormat) {
      console.log('Header format validation failed:', { headerA1, headerB1, headerC1 });
      return {
  ...
  private parseExcelFile(file: Express.Multer.File): ExcelQuestion[] {
    ...
    const jsonData = XLSX.utils.sheet_to_json<ExcelQuestion>(worksheet, {
      header: ['title', 'type', 'options', 'scores', 'isRequired', 'placeholder', 'minNum', 'maxNum', 'others', 'mustOthers', 'othersKey', 'placeholderDesc', 'layout', 'quotaDisplay', 'showIndex', 'showType', 'showSpliter'],
      range: 1, // 从第二行开始读取数据
    });
  ...
  async exportSurvey(
    ...
    try {
      ...
      if (!surveyMeta || !surveyConf) {
        throw new HttpException('问卷不存在或已删除', EXCEPTION_CODE.RESPONSE_SCHEMA_REMOVED);
      }
    ...
    // 检查数据结构
    if (!exportData.surveyConf || !exportData.surveyConf.dataConf || !exportData.surveyConf.dataConf.dataList) {
      this.logger.error(`exportSurvey data structure error: surveyConf=${JSON.stringify(exportData.surveyConf)}`);
      throw new HttpException('问卷数据结构错误', EXCEPTION_CODE.PARAMETER_ERROR);
    }
          ...
          // 创建分页配置表
          const pageConfData = [
            { 字段: '分页配置', 值: (exportData.surveyConf as any).pageConf ? (exportData.surveyConf as any).pageConf.join(',') : '' },
          ...
          // 创建描述配置表
          const descriptionConfig = (exportData.surveyConf as any).bannerConf?.descriptionConfig;
          ...
          // 创建题目数据表 - 与导入格式保持一致
          const questionsData = exportData.surveyConf.dataConf.dataList.map((question) => {
            // 题型映射：英文转中文
            const typeMapping: Record<string, string> = {
              'text': '单行输入框',
              'textarea': '多行输入框',
      ...
      if (question.options && question.options.length > 0) {
        questionData.选项内容 = question.options.map(option => option.text).join('; ');
        questionData.选项分数 = question.options.map(option => (option as any).score || '').join('; ');
        
        // 处理选项的特殊字段
        const othersOptions = question.options.filter(option => option.others);
        if (othersOptions.length > 0) {
          questionData.其他选项 = othersOptions.map(option => option.text).join('; ');
          questionData.其他选项必填 = othersOptions.map(option => option.mustOthers ? '是' : '否').join('; ');
          questionData.其他选项键值 = othersOptions.map(option => option.othersKey || '').join('; ');
          questionData.提示内容 = othersOptions.map(option => option.placeholderDesc || '').join('; ');
        }
      }
